    ...
    fetch(`/api/auto/${_id}`)
        ...
        .then(autos => {
            if (autos.length === 0) {
                ...
                // auto's renderen:
                const likedResult = document.querySelector('ul.liked-cars')
                likedResult.innerHTML = ''
                for (const auto of autos) {
                    const li = document.createElement('li');
                    li.addEventListener('click', () => {
                        window.location = `/detail/${auto._id}`
